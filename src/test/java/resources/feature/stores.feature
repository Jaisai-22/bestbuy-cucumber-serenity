Feature: Testing different requests on the best buy stores application
  ...
  Scenario: Verify if the best buy stores application can be accessed by users
    When User sends a GET request to the stores endpoint, they must get back a valid status code 200

  Scenario Outline: Create a new stores and verify if the stores is added
    ...
    Examples:
      | lat       | lng        | hours                                                                  |
      | 23.124356 | -89.332456 | Mon: 9-6; Tue: 9-6; Wed: 9-6; Thurs: 9-6; Fri: 9-6; Sat: 9-6; Sun: 9-6 |
  ...
  Scenario: Update a created store and verify if the store is updated
    When I update a created store providing the new name type and hours
    Then I verify the store is updated

  Scenario: Delete a created store and verify the store is deleted
